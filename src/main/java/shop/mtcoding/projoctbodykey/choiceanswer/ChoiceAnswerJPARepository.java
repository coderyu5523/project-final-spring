...
public interface ChoiceAnswerJPARepository extends JpaRepository<ChoiceAnswer, Integer> {
    ...
    @Query("select new shop.mtcoding.projoctbodykey.admin.survey.AdminSurveyRequest$ChoiceCountDTO(ca.questionChoice.id, COUNT(ca)) FROM ChoiceAnswer ca WHERE ca.surveyQuestion.id = :questionId GROUP BY ca.questionChoice.id")
